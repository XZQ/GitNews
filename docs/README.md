...
本目录收纳当前产品边界、路线图、历史实施计划、接口样例、审计记录和截图资产。当前开发基线为 `1.4.0+4` 加 `Unreleased` 改动；判断当前能力时依次以实际代码、根目录 README、产品与数据方案、RUN 为准。旧审计、旧规格和旧实施计划均保留为历史决策记录，文件顶部会明确标注“历史快照”，其中的旧版本号、旧品牌或旧导航不代表当前要求。
