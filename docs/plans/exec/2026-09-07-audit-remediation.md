...
状态：Complete（仓库内实施与本地验收完成；外部服务边界见下文）
...
- [x] (2026-09-12) S13 连续选文/复制收录正文，按真实字段展示完整 AI 摘要、翻译与实体；关联仓库使用现有壳内详情、订阅持久化并可返回阅读；完整 all Harness 14/14 通过。
...
- [x] (2026-09-12 08:20Z) 最终跨页面、浅深主题、中英文、紧凑窗口与大字号回归完成；all Harness 14/14，553 项 Flutter、20 项服务端测试通过。18 个修复提交已逐项推送，最终实现 `62b883b` 的本地、origin/main 和远端 SHA 相同，分歧 `0 0`。
...
S01–S15 的仓库内实施和本地验收已完成，共 18 个独立修复提交，均在适用门禁通过后逐项推送到 `main`。最后补充本验收记录，不改变已通过最终 all Harness 的运行时代码。

最终结果覆盖：发布启动与 SQLite 绑定、真实 Star 观测和总览口径、中文检索与分页、强制刷新和降级缓存、离线示例详情、兴趣排序、稍后读筛选、矮窗口加载布局、后台确认与失败重试、监控观测状态、服务端 AI 凭据边界、多来源访问，以及连续阅读和仓库订阅。

外部边界仍明确保留：AI 代理需要部署者配置真实 Supabase/Agnes 服务并部署；旧发行包内的共享 Key 需在服务商侧撤销轮换。本次未验证外部代理或真实模型调用、真实系统通知送达、Android 真机行为。用户停止原生 Computer Use 后未恢复该通道；页面布局证据来自 Widget/Golden，Release 烟测仅证明初始化成功、存储可读与托盘进程存活。
...
| S08 稍后读 | `50bd197` |
| S09 矮窗口加载 | `f9167a0` |
| S10a/b 后台确认与验证时间 | `464318d`、`64539ee` |
| S15a/b/c 监控持久快照与状态 | `0536af8`、`508c2e8`、`503438d` |
| S11 AI 服务端代理 | `a2aeeb2` |
| S14 总览指标与布局 | `234d670` |
| S12 多来源列表 | `b7d3a36` |
| S13 阅读与仓库订阅 | `62b883b` |
...
- S13：`build/harness/20260912T081550045Z-14804/summary.json`，all 14/14；全量 Flutter 553/553、服务端 20/20，Windows Release、存储可读/主壳首帧、托盘存活和服务端回环通过。28 项聚焦检查覆盖复制正文/摘要回退、完整生成文案、320px 双倍字号中英文阅读、实际 AI 子路由、真实本地订阅控制器持久化和返回文章。检查四张详情测试渲染后更新基线；同一固定上游字体补齐新文案缺字，字体与渲染只用于测试，不进入发行包。
