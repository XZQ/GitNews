...
- [x] (2026-09-12) S10b 后台缓存保留真实验证时间：沿用来源时间，stale 保留旧时间并持久化失败状态，无日期不补当前时间，seed 不写远端缓存；desktop Harness 10/10 通过。
- [x] (2026-09-12) S11 共享 AI 凭据迁至服务端：客户端只配置公开代理地址和用户会话，服务端验证账号、原子预占用户/全局额度、限制文章字段与响应大小；完整 all Harness 14/14 通过。
- [x] (2026-09-12) S12 AI 列表可读性与来源访问：两端可展开全部相关报道，分别显示篇数与来源数，逐篇可达；保持兴趣排序与搜索顺序，增加来源热度说明和稳定列表 Key；desktop Harness 10/10 通过。
- [ ] S13 详情阅读、AI 语义、来源与仓库跟踪闭环。
- [x] (2026-09-12) S14 总览布局与指标：卡片按宽度/字号换行，未知值与真实零值区分；移除评分与百分比相加，Star 净变化显示日期和固定样本覆盖数；desktop Harness 10/10 通过。
- [x] (2026-09-12) S15a 监控强制刷新失败保留持久快照：失败保留缓存且持久化失败状态，首次离线保留用户仓库，无示例替换；desktop Harness 10/10 通过。
- [x] (2026-09-12) S15b 部分失败保留完整监控列表和各仓库观测：按仓库持久化尝试/成功时间和失败原因，仅成功响应生成观测，摘要部分失败不推进验证时间；desktop Harness 10/10 通过。
- [x] (2026-09-12) S15c 真实检查时间、失败状态与重试：监控桌面/移动/详情及总览共用检查状态；刷新合并重复点击并发布本次结果，未知指标显示占位，空搜索保留输入；desktop Harness 10/10 通过。
...
- 2026-09-12：监控数据状态落库后先完成 S15c 的可见状态与重试，再处理 S11。复用同一检查契约，避免数据层已修复而页面继续显示固定“正常”。
...
- S12：`build/harness/20260912T075618031Z-21668/summary.json`，desktop 10/10；13 项聚焦回归覆盖同一来源多篇报道、全部成员详情、展开不触发主报道、兴趣重排保留状态、独立搜索结果及浅深/窄屏双倍字号。移动来源行字号、换行与收藏点击区域调整后，检查测试渲染差异并只更新 `ai_news_page_mobile.png`；这是组件渲染，不是真实窗口截图。
- S14：`build/harness/20260912T074517029Z-8668/summary.json`，desktop 10/10；15 项聚焦回归覆盖已加载数量、仓库去重、负净变化、历史缺失、加载/失败/空态、浅深主题、中英文、360/760/1280 宽度、双倍字号和全部五个入口导航。
- S11：`build/harness/20260912T073551260Z-2148/summary.json`，all 14/14；服务端 12 项代理回归包含鉴权、并发额度、无部分扣额、输入限制与上游失败，客户端补退出登录后缓存可读且无法重新生成的回归。初次分析失败和两张 golden 的旧认证 fixture 已修正，原图基线未改动。公开资讯不依赖代理，模型 Key 已移出 Flutter 构建；真实 Supabase/Agnes 调用、外部部署及旧 Key 撤销轮换需发布方环境，本地门禁不代表这些已完成。
- S15c：`build/harness/20260912T071103755Z-19352/summary.json`，desktop 10/10；17 项聚焦回归覆盖刷新等待与恢复、浅深主题/中英文大字号、移动/桌面失败状态及空搜索恢复。首次门禁因两个文件格式未稳定失败，补格式化后重跑完整门禁通过；未更新视觉 golden 基线。
- S15b：`build/harness/20260912T065755897Z-20724/summary.json`，desktop 10/10；26 项聚焦测试通过，包含部分成功后重建、首次部分失败、失败原因持久化、限流门禁及仓库重定向保持订阅标识。旧缓存缺少逐仓库时间时保留未知，不虚构成功时间。
- S15a：`build/harness/20260912T064906101Z-29568/summary.json`，desktop 10/10；6 项仓库回归包含 TTL 内强刷失败后重建、成功恢复替换、首次离线和空监控集合。部分失败和检查状态仍由 S15b/c 完成。
- S10b：`build/harness/20260912T064352244Z-19196/summary.json`，desktop 10/10；后台刷新测试 10/10，包含新增的来源时间、TTL 内失败后重新读取、无日期 stale 和 seed 隔离回归。
