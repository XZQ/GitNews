...
1. 总览：跨模块摘要与重点信号，顶部展示 AI HOT 当前热点，并整合 GitHub 热榜三个内容区块和 AI 雷达五个内容区块。
2. AI：AI 动态。
3. 发现：承接流行仓库、Agent Skills、官方账号和人物发现。
4. 监控：直接进入仓库观测、规则和应用内告警闭环。
5. 我的：本地收藏、关注、主题、Token 和数据管理。

移动端不是桌面侧栏的缩小复刻；底部导航固定为五个用户目的地。当前总览先以 AI HOT 当前热点帮助用户判断今天最值得看的变化，再以 8 个内容区块承接 GitHub 热榜的 Star 增长榜、热门仓库、话题趋势，以及 AI 雷达的标签、Agent 榜、信号热度、语言占比和五个主题；两个完整页面继续保留独立二级路由，但不占用底部导航位置。监控告警和开发者推荐分别留在监控、发现目的地，不重复占用总览篇幅。
...
- 默认读取 TTL 内的有效缓存；缓存缺失、过期或显式刷新时请求远端。
...
- 手动刷新只影响当前查询或资源，不清空无关缓存。资讯流刷新穿过条目、REST 和 RSS 的 TTL，继续使用 ETag/Last-Modified 条件请求；等待本次检查完成。搜索与稍后读视图只重新查询对应本地数据。
- 本地资讯库搜索：英文走 FTS5/BM25，连续中文按字面子串匹配；按 50 条分页，来源/分类/日期/已读过滤作用于每页。搜索结果逐篇保留检索顺序，不由兴趣排序或聚类隐藏；翻页失败保留已载内容并可重试。
...
- `estimated`：有明确模型或样本推断依据的估算值；不用于合成 Star 增长。
- `unavailable`：缺少可计算指标的观测历史。
- `seed`：种子数据自带的展示值。

Star 历史携带 UTC 日期。1/7/30 天窗口按日期筛选；跨仓库趋势只使用至少两个共同观测日期的固定样本，以首个共同观测日为基准计算累计净变化，保留负值和采样间隔，显示实际日期及覆盖数。旧缓存没有日期时不可用于增长图。
