...
- 离线首启示例通过独立只读通道提供列表和详情，按分类筛选；不写入远端缓存或成功时间。列表与详情明确标记“离线示例”，不显示为当天新闻、不为示例生成 AI 解读。
- 资讯缓存沿用来源真实验证时间；TTL 命中、失败回退和种子不得写成当前成功时间。失败状态独立保存，直到成功验证后解除；任一启用来源失败时显示部分更新提示，列表标明最近完整验证时间。条目缓存 v4 保留原条目，但不信任旧版可能被失败刷新推进的 v3 时间戳。
...
- 普通资讯流先聚合事件，再按代表条目的兴趣反馈排序；日期分组和排序都使用本地自然日，较新日期优先。同日“不感兴趣”降低排序，偏好更新即时反映在最终列表；搜索保持数据库顺序。
- 稍后读直接检索完整实体快照，分类、关键词、来源、日期和已读条件组合生效；日期为半开区间，关键词按空白分词后做 AND 字面子串匹配。清理资讯缓存不影响稍后读的内容、来源筛选和详情；每篇独立展示，按加入时间倒序。已读/取消稍后读后立即更新相应列表。
...
监控始终保留用户选择的完整仓库集合。强制刷新仅绕过 TTL，成功后才替换已观测值；部分失败保留失败仓库的旧指标，只给成功仓库追加观测和计算告警。每个仓库分别持久化尝试时间、最后成功时间和有限失败原因；摘要任一仓库失败即标记为降级，失败不能推进整份摘要的成功验证时间。没有历史的仓库使用“未观测”口径，不代入示例指标。
