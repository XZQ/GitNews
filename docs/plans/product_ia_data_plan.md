...
1. 总览：跨模块摘要与重点信号，直接整合 GitHub 热榜三个内容区块和 AI 雷达五个内容区块。
2. AI：AI 动态。
3. 发现：承接流行仓库、Agent Skills、官方账号和人物发现。
4. 监控：直接进入仓库观测、规则和应用内告警闭环。
5. 我的：本地收藏、关注、主题、Token 和数据管理。

移动端不是桌面侧栏的缩小复刻；底部导航固定为五个用户目的地。当前总览以 8 个内容区块承接 GitHub 热榜的 Star 增长榜、热门仓库、话题趋势，以及 AI 雷达的标签、Agent 榜、信号热度、语言占比和五个主题；两个完整页面继续保留独立二级路由，但不占用底部导航位置。监控告警和开发者推荐分别留在监控、发现目的地，不重复占用总览篇幅。
