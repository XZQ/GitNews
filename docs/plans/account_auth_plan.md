...
2. 首发固定提供邮箱验证码、GitHub 和 Google；三者归属于同一个应用账号，可在登录后绑定。
3. 不提供手机登录，避免引入短信供应商、地区合规、费用和反滥用边界。
...
5. 推荐使用 Supabase Auth 作为身份代理，客户端使用 PKCE 和系统浏览器完成 OAuth；应用自己的 FastAPI 服务只验证用户 JWT，并继续负责同步、协作和业务数据，不自行保存密码或邮箱验证码。
6. GitHub 登录身份与 GitHub API 连接是两件事。登录方式只证明应用账号身份；现有 PAT / GitHub OAuth Token 继续作为设备上的 GitHub 数据源凭据，不能因应用登录而被隐式复用、上传或删除。

Supabase Auth 的官方能力覆盖邮箱 OTP、Google、GitHub、身份绑定与 Flutter 桌面 OAuth。Google 官方为桌面安装应用推荐 PKCE；GitHub 也建议原生公共客户端优先使用授权码 + PKCE，而不是把 Device Flow 当作普通登录首选：
...
- 不把 Supabase service role key、OAuth client secret 或邮件供应商密钥放入客户端。
...
|---|---|---|---|---|---|---|
| 账号入口 `/profile/login` | 选择登录方式或继续匿名 | 邮箱验证码登录 | Google、GitHub、查看隐私说明、继续匿名 | 我的用户卡、桌面侧栏用户卡 → 返回原页面 | 二级 | 是 |
...
1. 清楚标题“登录 AI资讯”；
2. 邮箱验证码主表单；
3. Google 与 GitHub 第三方登录按钮；
4. “继续匿名使用”；
5. 一句用途和隐私说明。

PAT、GitHub API 配额、LLM Key 和自托管 Key 不出现在账号登录页。
...
用户实体至少包含稳定 `userId`、显示名、头像、已验证邮箱的脱敏值、绑定身份列表和创建时间。UI 不以 provider 的可变用户名作为本地数据主键。
...
- refresh token 使用 `FlutterSecureStorage`；普通用户资料可缓存，但日志、埋点和异常中不得出现 token、完整邮箱或验证码。
...
| 邮件验证码 | 认证服务 + 邮件投递供应商 | 一次性、短时有效 | 允许重发 | 限频后显示可重试时间 | 已发送到脱敏邮箱 |
...
| 验证码错误 | 保留邮箱，不清空整页 | 不改变当前会话 | 修改验证码或重发 |
...
验证码页还必须覆盖完整邮箱过长、6 位粘贴、系统大字体、窄屏键盘遮挡、深浅主题和中英文错误文案。
...
- 邮箱输入与发送按钮是首屏主表单；Google、GitHub 按钮紧随其后。
...
第一条切片采用“邮箱验证码登录 → 会话恢复 → 账号中心 → 登出”，先证明账号地基，再接通第三方 provider 的真实回调。
...
2. 输入真实测试邮箱并发送验证码。
3. 输入正确验证码，收到应用账号会话。
4. 返回“我的”，用户卡显示已验证邮箱，并明确 GitHub 数据源仍是未连接/已连接的独立状态。
...
- 确定邮件投递供应商、自有发件域名、限频和退信监控。

### 阶段 1：邮箱账号客户端垂直切片

- 替换本地假会话，完成邮箱 OTP、会话恢复、账号用户卡和登出。
- 建立账号入口、回调处理和安全存储适配。
...
- 接入邮箱 OTP 与自有域名 SMTP。
- 完成 CAPTCHA、IP/设备/邮箱限频、投递告警和退信监控。
- 验证目标网络环境下的邮件可达率后才公开入口。
...
- [ ] 登录页只提供邮箱、Google、GitHub，不存在手机入口或相关构建开关。
...
- [ ] 客户端包中没有 provider secret、service role key、master key 或邮件供应商密钥。
- [ ] token、验证码、完整邮箱不进入日志、分析事件、截图、fixture 或导出。
- [ ] 发送邮箱 OTP 有 CAPTCHA、枚举保护、IP/设备/目标地址限频和告警。
...
- [ ] Android 真机完成邮箱、Google、GitHub 真实登录。
...
3. 邮箱、GitHub、Google 是固定的三种登录方式，不再设置单独的客户端能力开关。
4. 当前仓库没有生产 Supabase 项目、域名、SMTP 或 OAuth 应用凭据，因此只能完成客户端实现、Fake 验证和未配置降级，不能声称真实邮箱 OTP 或 OAuth 已上线。

生产入口启用前仍需补齐真实邮件投递、CAPTCHA、限频、回调注册、投递监控和真机验收。
