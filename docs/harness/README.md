...
- 启动烟测通过 `GITHUB_NEWS_STARTUP_REPORT` 为本次进程记录受控状态，要求
  本地资讯表可读、初始化成功和主壳首帧完成；错误恢复页不能通过。普通运行
  不写探针文件，探针不记录异常原文或用户内容。它不替代页面内容与视觉验收。
- 目前没有自动采集当前 UI 截图或视频；托盘烟测证明窗口关闭后进程存活，
  不证明通知实际送达。
