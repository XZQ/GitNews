...
状态：已通过（一次性 QA 记录，2026-08-09 归档至 docs/audits/）

## Source of truth

- 设计稿原图：已不可得（原文件位于本机 `Downloads\AI咨询App界面重设计\`，已清理）。
- 现存比对证据：归一化设计稿 `artifacts/design-qa-ai-detail/source-mobile-390.png` + 实现 Golden `test/features/ai_news/presentation/goldens/ai_news_detail_design_target.png`。
