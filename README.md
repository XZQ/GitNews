...
- AI 资讯支持设置页源管理、连续失败健康状态、FTS5 全文检索、来源/时间/已读过滤、多源事件聚类、本地兴趣排序，以及逐条 LLM 摘要、翻译、重要性评分和实体缓存；REST 与 RSS 按 AI HOT ID/permalink 去重，RSS 补充作者、分类、原文和可再分发正文。
...
当前开发基线为 `1.4.0+4` 加 `Unreleased` 改动。阶段一的多源聚合、资讯库和资讯↔GitHub 已经扩展到完整的阶段二客户端能力：独立 Settings、源管理、可选的逐条 LLM 增强、事件聚类、FTS5、兴趣反馈、托盘与提醒均已落地。新的 `server/` 边界已经以可选、自托管方式实现定时采集、同步、协作、推送衔接和 GH Archive 分析；外部移动推送仍需部署者提供真实平台凭据，仓库不伪造已上线状态。
