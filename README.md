...
`1.3.0+3` 已完成无服务端前提下的可信本地闭环：真实集合快照、按日监控口径、凭据和仓库集合隔离缓存、真实 GitHub 活动、OAuth/PAT 构建降级、启动恢复、事务化配置导入，以及 Windows Release 启动烟测形成同一条可验证链路。服务端定时采集、系统推送和跨设备同步属于下一阶段的新系统边界，不在当前客户端能力中伪装实现。
