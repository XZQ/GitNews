...
- AI 资讯支持设置页源管理、连续失败健康状态、FTS5 全文检索、来源/时间/已读过滤、多源事件聚类、本地兴趣排序，以及内置 Agnes 的逐条摘要、翻译、重要性评分和实体缓存；最终用户无需配置模型，真实生成失败时详情页不展示 AI 深度解读。REST 与 RSS 按 AI HOT ID/permalink 去重，RSS 补充作者、分类、原文和可再分发正文。
