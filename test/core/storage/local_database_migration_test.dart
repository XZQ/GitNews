...
*插入真实数据,然后调用 `onUpgradeSchema(db, 1, 6)` 跑完整迁移链,
*验证:
*- 所有 v6 表已创建;
*- v1 已存在的数据未丢;
*- ai_news_state 等「用户状态」表在新库中可用。
*
*另外覆盖 v2→v6、v3→v6、v4→v6、v5→v6 部分链路。
*/
void main() {
  setUpAll(() {
    sqfliteFfiInit();
  ...
  group('onUpgradeSchema full chain v1→v6', () {
    test('v1 → v6 creates all current-version tables', () async {
      ...
      for (final table in expected) {
        expect(names, contains(table), reason: '$table 必须在 v1→v6 升级后存在');
      }
    });

    test('v1 → v6 preserves legacy data and builds the FTS index', () async {
      final db = await openRawV1Db();
      addTearDown(db.close);
      await seedV1Data(db);
    ...
    test('v1 → v6 makes ai_news_state writable for user read-later state', () async {
      final db = await openRawV1Db();
      addTearDown(db.close);
      await onUpgradeSchema(db, 1, 6);
  ...
  group('onUpgradeSchema partial chains', () {
    test('v2 → v6 adds all later tables', () async {
      final db = await openRawV1Db();
      addTearDown(db.close);
      ...
      await onUpgradeSchema(db, 2, 6);
      final tablesAfterV6 = await tableNames(db);
      expect(
        tablesAfterV6,
    ...
    test('v3 → v6 adds monitor and AI news state tables', () async {
      final db = await openRawV1Db();
      addTearDown(db.close);
      await onUpgradeSchema(db, 1, 3);

      await onUpgradeSchema(db, 3, 6);
      final tables = await tableNames(db);
      expect(tables, containsAll(['monitor_alert_event', 'ai_news_state', 'ai_news_fts']));
    });

    test('v4 → v6 adds user state and enrichment tables', () async {
      final db = await openRawV1Db();
      addTearDown(db.close);
    ...
    test('v5 → v6 adds FTS, enrichment and feedback tables', () async {
      final db = await openRawV1Db();
      addTearDown(db.close);
      await onUpgradeSchema(db, 1, 5);

      await onUpgradeSchema(db, 5, 6);
      final tables = await tableNames(db);
      expect(
        tables,
        containsAll(['ai_news_fts', 'ai_news_enrichment', 'ai_news_feedback', 'ai_news_reminder']),
