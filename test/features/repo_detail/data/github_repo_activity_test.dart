...
void main() {
  ...
  test('parses supported GitHub activity event types from real payload fields', () {
    final events = [
      ...
      (json) => parseGitHubRepoActivity(
        json,
        fallbackRepoFullName: 'fallback/repo',
      ),
    );
    ...
    expect(
      ...
      [
        RepoActivityType.push,
        RepoActivityType.issues,
        RepoActivityType.pullRequest,
        RepoActivityType.release,
        RepoActivityType.other,
      ],
    );
  ...
  test('github detail repository loads activities from repository events', () async {
    ...
    final cache = JsonSnapshotCacheDao(
      database.executor,
    ...
    when(
      ...
      (invocation) async => Response<Map<String, Object?>>(
