...
void main() {
  test('feedback persists and influences same-day ranking', () async {
    ...
    await dao.set(AiNewsFeedbackEntry(itemId: 'paper', signal: AiNewsFeedbackSignal.more, topicKey: AiNewsCategory.paper.code, updatedAt: now));

    final entries = await dao.readAll();
    final profile = AiNewsInterestProfile(itemSignals: {entries.single.itemId: entries.single.signal}, topicWeights: {entries.single.topicKey: entries.single.signal.value});
    final ranked = rankAiNewsByInterest([_item('model', AiNewsCategory.aiModels, now), _item('paper', AiNewsCategory.paper, now.subtract(const Duration(hours: 1)))], profile);
...
AiNewsItem _item(String id, AiNewsCategory category, DateTime publishedAt) {
  return AiNewsItem(id: id, category: category, title: id, titleEn: '', summary: '', source: '', url: '', permalink: '', publishedAt: publishedAt, score: 0, selected: false);
}
