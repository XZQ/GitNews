...
void main() {
  testWidgets('官方日报卡展示来源、版本并打开指定日期详情', (tester) async {
    final report = AiHotDailyReport(
      ...
      lead: const AiHotDailyLead(title: 'AI 模型与智能体进入新阶段', paragraph: '本期聚焦模型发布、智能体工具与行业应用。'),
      sections: const [
        AiHotDailySection(
          label: '模型',
          items: [AiHotDailyItem(title: '新模型发布', summary: '模型能力获得更新。', sourceUrl: 'https://example.com/original', sourceName: 'Example')],
        ),
      ],
      flashes: const [AiHotDailyFlash(title: '行业快讯', sourceName: 'Example', sourceUrl: 'https://example.com/flash', publishedAt: null)],
    );
    final router = GoRouter(
      routes: [
        GoRoute(
          path: '/',
          builder: (_, __) => const Scaffold(body: AiHotDailyCard()),
        ),
        GoRoute(
    ...
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          aiHotLatestDailyProvider.overrideWith((ref) async => DataResult(data: report, freshness: DataFreshness.live)),
