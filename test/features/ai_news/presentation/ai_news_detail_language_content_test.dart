...
void main() {
  test('Chinese source content does not create duplicate bilingual cards', () {
    final content = AiNewsDetailLanguageContent.fromItem(_item(title: '国务院推进人工智能全学段教育', titleEn: '国务院：推进人工智能全学段教育，提升学生人工智能素养', summary: '规划要求完善科学教育体系，强化科技教育与人文教育协同。'));
  ...
  test('English source content keeps its original and embedded Chinese translation', () {
    final content = AiNewsDetailLanguageContent.fromItem(_item(title: 'OpenAI 报告：绘制欧洲 AI 劳动力机遇版图', titleEn: "Mapping Europe's AI Workforce Opportunity", summary: 'OpenAI 发布新报告，分析 AI 对欧盟就业的影响。'));
  ...
  test('local English feed uses cached enrichment for the Chinese translation', () {
    final item = _item(title: 'A safer memory layer for coding agents', titleEn: '', summary: 'The release supports local storage and encrypted synchronization.');
...
AiNewsItem _item({required String title, required String titleEn, required String summary}) {
  return AiNewsItem(
    id: 'language-content',
