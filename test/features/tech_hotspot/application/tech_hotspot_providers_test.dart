...
void main() {
  group('techHotspotDigestProvider', () {
    test('local digest result is explicitly marked as seed data', () async {
      final container = ProviderContainer(overrides: [techHotspotRepositoryProvider.overrideWithValue(const LocalTechHotspotRepository())]);
      addTearDown(container.dispose);

      final result = await container.read(techHotspotDigestResultProvider.future);
    ...
    test('should propagate error when repository throws', () async {
      ...
      final container = ProviderContainer(overrides: [techHotspotRepositoryProvider.overrideWithValue(repo)]);
      addTearDown(container.dispose);

      await expectLater(container.read(techHotspotDigestProvider.future), throwsA(isA<StateError>()));
    });
  ...
  group('tech hotspot search', () {
    test('filterTechTopics should match topic name category summary and id', () {
      ...
      expect(filterTechTopics(_sampleDigest.topics, 'agent').single.id, 'agent-runtime');
      expect(filterTechTopics(_sampleDigest.topics, 'ollama').single.id, 'local-inference');
      expect(filterTechTopics(_sampleDigest.topics, 'model').single.id, 'local-inference');
    ...
    test('filteredTechHotspotDigestProvider should filter current digest', () async {
      final repo = _MockTechHotspotRepository();
      when(repo.getDigest).thenAnswer((_) async => const DataResult(data: _sampleDigest, freshness: DataFreshness.live));

      final container = ProviderContainer(overrides: [techHotspotRepositoryProvider.overrideWithValue(repo)]);
      addTearDown(container.dispose);

      container.read(techHotspotSearchQueryProvider.notifier).state = 'mcp';
      final digest = await container.read(filteredTechHotspotDigestProvider.future);
