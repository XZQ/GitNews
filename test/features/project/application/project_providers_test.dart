...
void main() {
  test('project result exposes repository response freshness', () async {
    ...
    when(repo.getDigest).thenAnswer(
      (_) async => DataResult(
        ...
        data: ProjectDigest(repos: [_repo('openai/codex')], contributors: const [], primaryTrend: const [], secondaryTrend: const [], activities: const []),
      ),
    );
  ...
  group('project search', () {
    ...
    test('filterProjectContributors should match login and contributions', () {
      final contributors = [_contributor('gaearon', contributions: 128), _contributor('sindresorhus', contributions: 64)];

      expect(filterProjectContributors(contributors, '').length, 2);
      expect(filterProjectContributors(contributors, 'gaearon'), [contributors.first]);
      expect(filterProjectContributors(contributors, '64'), [contributors.last]);
      ...
      when(repo.getDigest).thenAnswer(
        (_) async => DataResult(
          ...
          data: ProjectDigest(
            ...
            contributors: [_contributor('codex-maintainer'), _contributor('frontend-dev')],
            primaryTrend: const [],
