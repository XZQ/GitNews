...
void main() {
  ...
  const allRules = {MonitorRuleIds.starDailyDelta, MonitorRuleIds.starDailyRate, MonitorRuleIds.forkDailyDelta, MonitorRuleIds.issueHeatRatio};

  test('first observation establishes a baseline without alerts', () {
    final events = evaluator.evaluate(previous: null, current: observation(day: 2), enabledRuleIds: allRules);
  ...
  test('thirty day growth is normalized before daily thresholds', () {
    ...
    expect(events.map((event) => event.ruleId), isNot(contains(MonitorRuleIds.starDailyRate)));
  });
  ...
  test('reverse observation dates do not trigger', () {
    final events = evaluator.evaluate(previous: observation(day: 2, stars: 1000), current: observation(day: 1, stars: 5000), enabledRuleIds: allRules);
