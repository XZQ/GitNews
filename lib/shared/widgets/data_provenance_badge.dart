...
class DataFreshnessBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final keys = switch (freshness) {
      DataFreshness.live => ('provenance.live', 'provenance.live.full', 'provenance.live.tooltip'),
      DataFreshness.freshCache => ('provenance.fresh_cache', 'provenance.fresh_cache.full', 'provenance.fresh_cache.tooltip'),
      DataFreshness.staleCache => ('provenance.stale_cache', 'provenance.stale_cache.full', 'provenance.stale_cache.tooltip'),
      DataFreshness.seed => ('provenance.seed', 'provenance.seed.full', 'provenance.seed.tooltip')
    };
...
class _TrustBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Tooltip(
      message: tooltip,
      child: Container(
        padding: EdgeInsets.symmetric(horizontal: compact ? AppSpacing.xs2 : AppSpacing.sm, vertical: AppSpacing.xxs),
        decoration: BoxDecoration(
          color: inverse ? Colors.white.withValues(alpha: 0.18) : color.withValues(alpha: 0.1),
          border: Border.all(color: inverse ? Colors.white.withValues(alpha: 0.28) : color.withValues(alpha: 0.28), width: 0.7),
          borderRadius: BorderRadius.circular(AppRadius.pill),
        ),
        child: Text(label, style: AppTypography.labelMicro.copyWith(color: inverse ? Colors.white : color, fontWeight: FontWeight.w700, letterSpacing: 0)),
      ),
    );
  }
}
