...
class MonitorAlertsPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return SecondaryPageScaffold(
      ...
      body: state.when(
        data: (digest) {
          final events = ref.watch(monitorAlertEventsProvider).value ?? const [];
          ...
          return ResponsiveLayout(
            ...
            medium: (_) => CenteredContent(
              child: _Body(alerts: digest.alerts, archivedCount: archivedCount),
            ),
...
class _Body extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // 头部卡片 + Sliver 懒构建的告警列表:告警会随时间持久累积,
    // 旧的 shrinkWrap 列表会一次性构建全部条目,数量一多滚动必卡。
