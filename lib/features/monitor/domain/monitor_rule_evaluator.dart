...
class MonitorRuleEvaluator {
  ...
  List<MonitorAlertEvent> evaluate({required MonitorObservation? previous, required MonitorObservation current, required Set<String> enabledRuleIds}) {
    if (previous == null || previous.repoFullName != current.repoFullName) {
      return const [];
    }

    final elapsedDays = _elapsedLocalDays(previous.observedAt, current.observedAt);
    if (elapsedDays <= 0) {
      return const [];
    }
    ...
    final starRate = previous.stars <= 0 || current.stars <= previous.stars ? 0.0 : (math.pow(current.stars / previous.stars, 1 / elapsedDays).toDouble() - 1) * 100;
  ...
  int _elapsedLocalDays(DateTime previous, DateTime current) {
    ...
    final previousDay = DateTime(previousLocal.year, previousLocal.month, previousLocal.day);
    final currentDay = DateTime(currentLocal.year, currentLocal.month, currentLocal.day);
