...
class DiscoverRepository {
  ...
  Future<DataResult<List<DiscoverProfileEntity>>> fetchProfiles({
    ...
    final now = _now();
    final key = '${DiscoverQueries.profilesCache}:${kind.name}';
    if (force) {
      await _safeDelete(key);
    }
    if (!_blocked()) {
      if (!force && await _isFresh(key, CacheTtlConfig.discover, now)) {
        ...
        if (cached != null) {
          return DataResult(
            data: DiscoverCacheCodec.decodeProfiles(cached, kind),
            freshness: DataFreshness.freshCache,
          );
        }
      }
      try {
        ...
        for (final login in DiscoverQueries.profileLogins(kind)) {
          results.add(await _profileClient.fetch(login, kind));
        }
        final profiles = [for (final result in results) result.data];
        await _cache.upsert(
          key: key,
          payload: DiscoverCacheCodec.profilesToJson(profiles),
          now: now,
        );
        return DataResult(
          data: profiles,
