...
class AiNewsFeedbackEntry {
  const AiNewsFeedbackEntry({required this.itemId, required this.signal, required this.topicKey, required this.updatedAt});
...
class AiNewsInterestProfile {
  const AiNewsInterestProfile({this.itemSignals = const {}, this.topicWeights = const {}});
...
List<AiNewsItem> rankAiNewsByInterest(List<AiNewsItem> items, AiNewsInterestProfile profile) {
  ...
  ranked.sort((left, right) {
    ...
    final scoreOrder = _interestScore(right, profile).compareTo(_interestScore(left, profile));
