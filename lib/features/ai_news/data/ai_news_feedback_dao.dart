...
class AiNewsFeedbackDao {
  ...
  Future<List<AiNewsFeedbackEntry>> readAll() async {
    try {
      final rows = await _db.query('ai_news_feedback', orderBy: 'updated_at DESC');
      return rows.map(_fromRow).toList(growable: false);
    } catch (error, stack) {
      throw AppException(kind: AppExceptionKind.cache, cause: error, stack: stack, meta: {'op': 'readAiNewsFeedback'});
    }
  }

  Future<void> set(AiNewsFeedbackEntry entry) async {
    try {
      await _db.insert('ai_news_feedback', {
      ...
      throw AppException(kind: AppExceptionKind.cache, cause: error, stack: stack, meta: {'op': 'setAiNewsFeedback'});
    }
  }

  Future<void> remove(String itemId) async {
    try {
      await _db.delete('ai_news_feedback', where: 'item_id = ?', whereArgs: [itemId]);
    } catch (error, stack) {
      throw AppException(kind: AppExceptionKind.cache, cause: error, stack: stack, meta: {'op': 'removeAiNewsFeedback'});
    }
  }
