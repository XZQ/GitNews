...
class ProjectLanguageDistribution extends StatelessWidget {
  ...
  List<_LanguageSlice> _languages(List<RepoEntity> repos) {
    ...
    for (final repo in repos) {
      ...
      counts[repo.language] = _LanguageCount(count: (current?.count ?? 0) + 1, accentArgb: repo.accentArgb);
    }
...
class _LanguageList extends StatelessWidget {
  const _LanguageList({required this.languages, this.physics, this.shrinkWrap = false});
      ...
      itemBuilder: (context, index) {
        ...
        return Row(
          children: [
            SizedBox(
              width: 24,
              child: Text('${index + 1}', style: AppTypography.labelSmall.copyWith(color: Theme.of(context).colorScheme.onSurfaceVariant)),
            ),
            Container(
              width: 8,
              height: 8,
              decoration: BoxDecoration(color: Color(l.accentArgb), borderRadius: BorderRadius.circular(AppRadius.xs)),
            ),
            const SizedBox(width: AppSpacing.sm),
            Expanded(
              child: Text(l.name, style: AppTypography.bodyMedium, maxLines: 1, overflow: TextOverflow.ellipsis),
            ),
            const SizedBox(width: AppSpacing.sm),
            ...
            Text(
              ...
              style: AppTypography.labelSmall.copyWith(color: l.delta >= 0 ? AppColors.success : AppColors.danger, fontWeight: FontWeight.w600),
            ),
          ],
        );
      },
