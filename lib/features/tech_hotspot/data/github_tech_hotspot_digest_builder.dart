...
class GithubTechHotspotTopicResult {
  const GithubTechHotspotTopicResult({required this.topic, required this.languages, this.heatTrend});
...
List<LanguageStat> buildTechHotspotLanguages(List<GithubTechHotspotTopicResult> results) {
  final counts = <String, int>{};
  for (final result in results) {
    for (final entry in result.languages.entries) {
      counts.update(entry.key, (value) => value + entry.value, ifAbsent: () => entry.value);
    }
  }
...
List<TechHeatPoint> buildTechHotspotHeatTrend(List<GithubTechHotspotTopicResult> results) {
  ...
  if (observed.isNotEmpty) {
    ...
    return List<TechHeatPoint>.generate(pointCount, (index) {
      ...
      return TechHeatPoint(label: labels[index], value: (sum / observed.length).roundToDouble());
    });
  }
  ...
  return List<TechHeatPoint>.generate(labels.length, (index) {
    return TechHeatPoint(label: labels[index], value: (total / results.length * (0.76 + index * 0.05)).roundToDouble());
  });
}

List<String> buildTechHotspotTags(List<GithubTechHotspotTopicResult> results) {
  final tags = <String>[for (final result in results) result.topic.name, 'GitHub', 'Open Source', 'Repository', 'Tool Use', 'Inference', 'Vector DB'];
...
TechTopic copyTechHotspotTopic(TechTopic topic, {double? growth, MetricBasis? growthBasis}) {
  return TechTopic(
    id: topic.id,
    name: topic.name,
