...
class TechHotspotAgentSignalBoard extends StatelessWidget {
  const TechHotspotAgentSignalBoard({required this.topics, this.compact = false, super.key});
    ...
    return AppCard(
      child: Column(
        ...
        children: [
          Row(
            children: [
              if (!compact) ...[Icon(Icons.device_hub_rounded, size: 16, color: colors.primary), const SizedBox(width: AppSpacing.sm)],
              Expanded(
                child: Text(
          ...
          if (!compact) ...[const SizedBox(height: AppSpacing.xs), Text(l10n.tr('tech_hotspot.agent_board.subtitle'), style: AppTypography.bodySmall.copyWith(color: colors.onSurfaceVariant))],
            ...
            Row(
              ...
              children: [
                for (var i = 0; i < signals.length; i++) ...[
                  ...
                  Expanded(
                    child: _AgentSignalItem(rank: i + 1, topic: signals[i], compact: compact),
                  ),
                ],
              ],
            )
...
class _AgentSignalItem extends StatelessWidget {
  const _AgentSignalItem({required this.rank, required this.topic, required this.compact});
                ...
                Row(
                  children: [
                    ...
                    Text(
                      ...
                      style: AppTypography.labelSmall.copyWith(color: topic.growth >= 0 ? accent : AppColors.trendDown, fontWeight: FontWeight.w700),
                    ),
                  ],
                ),
