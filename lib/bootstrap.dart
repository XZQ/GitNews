...
Future<BootstrapResult> initializeApplication({
      ...
      builder: (context, snapshot) {
        ...
        final failure = result ??
            BootstrapResult.failure(
              snapshot.error ?? StateError('Unknown bootstrap failure'),
...
class _BootstrapLoadingView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Semantics(
        label: AppLocalizations.of(context).tr('bootstrap.loading'),
        ...
        child: ConstrainedBox(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                l10n.tr('bootstrap.failure.title'),
              ...
              FilledButton.icon(
                ...
                label: Text(l10n.tr('bootstrap.retry')),
              ),
              const SizedBox(height: AppSpacing.sm),
              OutlinedButton.icon(
  ...
  Future<void> _openDirectory(BuildContext context) async {
    ...
    if (!opened && context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            AppLocalizations.of(context).tr('bootstrap.open_failed'),
          ),
        ),
      );
    }
