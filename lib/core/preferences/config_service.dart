...
class ConfigService {
  ...
  Future<int> importConfig() async {
    ...
    if (text == null || text.trim().isEmpty) {
      throw FormatException('Clipboard is empty');
    }

    final decoded = jsonDecode(text);
    if (decoded is! Map<String, dynamic>) {
      throw FormatException('Invalid config format: expected JSON object');
    }

    final prefs = decoded['preferences'];
    if (prefs is! Map<String, dynamic>) {
      throw FormatException('Invalid config format: missing "preferences"');
    }
