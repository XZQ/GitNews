...
*那种场景应保留原项而非丢弃。
*/
Future<List<T>> gatherAll<T>(Iterable<Future<T>> futures, {Duration? timeout, String tag = 'gatherAll'}) async {
  ...
  final guarded = <Future<T?>>[for (var i = 0; i < list.length; i++) guard(i, list[i])];
  final results = await Future.wait<T?>(guarded, eagerError: false);
  final ok = results.whereType<T>().toList(growable: false);
  if (ok.isEmpty) {
    throw AppException(kind: AppExceptionKind.network, cause: StateError('all $tag calls failed (${list.length})'));
  }
  return ok;
}
