...
Future<DataResult<List<RepoActivityEvent>>> fetchGitHubRepoActivities({
  ...
  try {
    final result = await resources.getList(
    ...
    return result.map(
      (data) => data
          .map(
            (raw) => parseGitHubRepoActivity(
              raw,
              fallbackRepoFullName: fullName,
            ),
          )
...
RepoActivityEvent parseGitHubRepoActivity(
  Object? raw, {
  required String fallbackRepoFullName,
}) {
  ...
  return RepoActivityEvent(
    ...
    occurredAt: DateTime.parse(
      GitHubJson.string(json['created_at']),
    ).toUtc(),
...
_ParsedActivity _parseRelease(Map<String, Object?> payload) {
  ...
  return _ParsedActivity(
    ...
    title: _withAction(
      _nullableString(payload['action']),
      releaseTitle,
      fallback: 'ReleaseEvent',
    ),
...
_ParsedActivity _parseCreate(Map<String, Object?> payload) {
  ...
  return _ParsedActivity(
    type: RepoActivityType.create,
    title: details.isEmpty ? 'CreateEvent' : details,
    htmlUrl: null,
  );
}
